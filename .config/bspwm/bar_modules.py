...
class CoronaVirus():
    ...
    def worldometer_thread(self):
        try:
            ...
            print('downloaded worldometer page')
    ...
    def output(self):
        ...
        if cpu.startswith('Linux'):
            ...
            if cpu_usage >= 85:
                ...
                return '%{T2}'+self.icon+'%{T-}'+f'{cpu_usage:.1f}'
...
class NetworkTraffic():
    ...
    def get_ip_out(self):
        ...
        ifaces = [
            ...
            if not d['ifname'] in self.exclude and 'UP' in d['flags']]
        return ifaces
    ...
    def output(self):
        ...
        if len(ifaces) == len(self.cache):
            speeds = [
                (ifaces[i][0] if len(ifaces[i][0]) < 6 else ifaces[i][0][:6],
    ...
    def output(self):
        ...
        for w in all_workspaces:
            ...
            if w == current:
                ...
                formatted_ws.append(
                    pre1+w+':}'+'%{B'+cdict['red']+'}'+wor+'%{B}%{A}')
            elif w in empty_workspaces:
                formatted_ws.append(
                    pre1+w+':}'+'%{B'+cdict['cyan']+'}'+wor+'%{B-}%{A}')
            else:
                formatted_ws.append(
                    pre1+w+':}'+wor+'%{A}')
