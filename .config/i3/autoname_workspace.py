...
# remeber to include the util.py file
from util import *
...
def rename_workspaces(i3):
    ...
    for ws_index, workspace in enumerate(i3.get_tree().workspaces()):
        ...
        # an ugly try to group similar windows icons
        # icons_lst = []
        # for i, w in enumerate(workspace.leaves()):
        #     if i + 1 < len(workspace.leaves()):
        #         if icon_for_window(workspace.leaves()[i + 1]) != icon_for_window(w):
        #             current_icon = icon_for_window(w)
        #             icons_lst.append(current_icon)
        #         if icons_lst:
        #             if icons_lst[-1] != icon_for_window(workspace.leaves()[-1]):
        #                 icons_lst.append(icon_for_window(
        #                     workspace.leaves()[-1]))

        # if icons_lst:
        #     name_parts['icons'] = ' '.join(icons_lst)
        # else:
        #     name_parts['icons'] = ' '
        ...
        # This code is to group similar icons next to each other
        icns_lst = name_parts['icons'].split()
        if icns_lst:
            new_icns = [icns_lst[0]]
        else:
            new_icns = []
        if icns_lst:
            for idx, wicn in enumerate(icns_lst[1:]):
                if wicn == icns_lst[idx]:
                    continue
                else:
                    new_icns.append(wicn)

        name_parts['icons'] = ' '.join(new_icns)
