...
(set-fontset-font "fontset-proportional" 'arabic (font-spec :family "Greta Arabic Regular"))
...
(:name "Last month" :query "date:1m.." :key 58)
 (:name "Section Management" :query "from:dhiaa OR from:hassan OR from:reservoirs" :key 39)))
(set-face-attribute 'mu4e-header-face nil :inherit 'fixed-pitch :family "Dejavu Sans Mono")

(add-hook 'mu4e-main-mode-hook (lambda () (text-scale-set 2)))
(add-hook 'mu4e-header-mode-hook (lambda () (text-scale-set 2)))
)
